...
import processSchedule.UserModel;
import processSchedule.WorkingShift;
...
//            String to = "";
            ...
            if(lst != null && lst.size() > 0){
                ...
                for (int i = (lst.size() - 1); i >= 0; i--) {
                    ...
                    String shift16 = "";
                    String DateW = "";
                    String working = "";// se luu nhung ai truc hom day
                    cell = cell + 1;
                    ScheduleWorkModel.ShiftLst model = result.new ShiftLst();
                    ...
                    for (int j = 0; j <= (strGetStatus.length - 1); j++) {//0-15
                        ...
                        if (Integer.parseInt(strGetStatus[j].trim()) == 1) {// co hoat dong
                            ...
                            String avail = "V";
                            if (check.checkStatusRequest(Integer.parseInt(strGetID[j])) == 2) {// da co nguoi dat thanh cong
//                                out.println("<td style='background: #bbff84;height:30px' align=\"center\"><img src=\"img/available.png\" /></td>");
                                avail = "||";
                            } else if (check.checkStatusRequest(Integer.parseInt(strGetID[j])) == 0) {// dang co nhiu nguoi dat nhung chua approve
...
//                                out.println("<td style='background: #ececec;height:30px' align=\"center\"><a title=\"Total Request: " + totalRq.total(Integer.parseInt(strGetID[j])) + "\" href=\"?option=sendRequest&sid=" + strGetID[j] + "\"><img src=\"img/available.png\" /></a></td>");
                        ...
                        List<WorkingShift> lstWS = checkRequest.getListWSByDateID(studentDetailsDTO.getDateworkID());
                        String onDuty = "";
                        if(lstWS != null && lstWS.size() > 0){
                            for(int um = 0; um < lstWS.size() ; um ++){
                                UserModel userModel = checkRequest.getListUserByID(lstWS.get(um).getUser_id());
                                if(userModel != null){
                                    if(um == (lstWS.size() -1)){
    ...
    //        }
        ...
        private static List<ScheduleWorkModel.ShiftLst> getListShift(){
            List<ScheduleWorkModel.ShiftLst> lst = new ArrayList<ScheduleWorkModel.ShiftLst>();
