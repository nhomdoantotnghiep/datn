 ...
 * @author User
 */
public class checkCateName extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            String str = request.getParameter("name");
             int cID = Integer.parseInt(request.getParameter("cID").toString().trim());
            CheckUsername check=new CheckUsername();
            int result=check.checkUsername(str,"tbl_category","cateName"," and cateID!="+cID);
            if (result == 1) {
